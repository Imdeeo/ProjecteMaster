...
CDrawQuadRendererCommand::CDrawQuadRendererCommand(CXMLTreeNode &TreeNode) :CStagedTexturedSceneRendererCommand(TreeNode)
{
	...
	if (c != NULL)
	{
		m_Materials.push_back(UABEngine.GetMaterialManager()->GetResource(std::string(c)));
		m_RenderableObjectTechnique = m_Materials[0]->GetRenderableObjectTechnique();
	}
	else
	{
...
void CDrawQuadRendererCommand::Execute(CRenderManager &_RenderManager)
{
	if (Getactive())
	{
		...
		if (m_Materials.size()>0)
			m_Materials[0]->Apply(m_RenderableObjectTechnique);
