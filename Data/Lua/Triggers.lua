...
function OnEnterClimbing(_TriggerName, _ColliderName)
	local l_Player = m_CharacterManager.m_Player[1]
	--TODO: Set l_Player.m_Offset from trigger
	l_Player.m_Offset = Vect3f(1.0, 0.0, 0.0)
	l_Player.m_IsInteracting = false
...
function CheckIfFacing(_Player, _TriggerName)
	...
	if (_Player.m_CameraController:get_forward():get_normalized(1):get_angle_with(_Player.m_Offset) > 2.0 and (_Player.m_PhysXManager:get_character_controler_pos("player") - l_Target):get_normalized(1):get_angle_with(_Player.m_Offset) < 1.57) then
