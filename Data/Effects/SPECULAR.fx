...
float4 mainPS(TVertexPS IN) : COLOR
{
	...
	float3 Pixelpos = l_eyePos-IN.EyeToVec;
	// Light properties
	...
	// Intermediate values
	float3 l_RayDirection = normalize(Pixelpos - Lamp0Position);
	float l_Distance = distance(Pixelpos, Lamp0Position);
	...
	float3 l_ReflectVec = IN.Normal*l_RayDirection;
	float l_specularContrib = max(dot(l_ReflectVec,-IN.EyeToVec),0);
	...
	//return float4(IN.EyeToVec,1);
	float l_DifusseFactor = min(1,max(l_DiffuseContrib*l_DistanceAttenuation*l_SpotAttenuation,0));
	return tex2D(S0Sampler,IN.UV)*float4(Lamp0Color*l_DifusseFactor,1)+(float4(Lamp0Color*l_specularContrib*l_DifusseFactor, 1.0));
}
